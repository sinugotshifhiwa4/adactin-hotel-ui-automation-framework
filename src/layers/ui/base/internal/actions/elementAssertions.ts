...
export class ElementAssertions extends ActionBase {
    ...
    return this.performAction(
      async () => {
        switch (propertyType) {
          ...
          default:
            logger.error(`Unsupported property type: ${propertyType as string}`);
            throw new Error(`Unsupported property type: ${propertyType as string}`);
        }
      },
      callerMethodName,
  ...
  public async hasBackgroundColourClass(
    ...
    try {
      ...
      logger.info(`${elementName} background colour '${colour}' present: ${hasColour}`);
